...
namespace Remnant_Afterglow
{
	public partial class FixedTileMap : TileMap
	{
			...
			if (@event is InputEventMouseButton mouseButton)
			{
				if (mouseButton.Pressed)//点击
				{
					if (mouseButton.ButtonIndex == MouseButton.Left)//左键
					{
						...
						// 检查是否可以在此位置建造
						if (ObjectManager.Instance.CanCreateBuild(buildData, mapPos))//能否建造
						{
							if (BagSystem.Instance.RemoveBuildPrice(buildData, true))//建造资源是否足够
							{
		...
		/// </summary>
		/// <returns></returns>
		public Vector2 GetBuildPos(bool IsNum, Vector2 pos)
		{
			if (IsNum)//偶数
			{
				Vector2 mousePos = pos - new Vector2(20, 20);
		...
		/// </summary>
		/// <returns></returns>
		public Vector2 GetBuildPos(bool IsNum, Vector2I mapPos)
		{
			if (IsNum)//偶数
			{
				return mapPos * MapConstant.TileCellSize;
