            ...
            if (@event is InputEventMouse mouseEvent)// 检查是否是鼠标事件
            {
                if (isRange)// 检查鼠标是否在节点上
                {
                    ...
                    if (mouseEvent is InputEventMouseButton mb && mb.ButtonIndex == MouseButton.Left && mb.Pressed)
                    {
                        ...
                        if (IsCopy)//是关卡
                        {
                            GlobalData.PutParam("ChapterId", chapterCopy.ChapterId);//章节
                            GlobalData.PutParam("CopyId", chapterCopy.CopyId);//关卡
                            GetTree().ChangeSceneToFile("res://scenes/MainCopy.tscn");
                        }
                        else
                        {
