...
namespace Remnant_Afterglow
{
    ...
    public partial class BaseObject : CharacterBody2D, IPoolItem
    {
        ...
        /// </summary>
        /// <param name="id"></param>
        public Dictionary<string, AttributeData> attrDict = new Dictionary<string, AttributeData>();
            ...
            foreach (var attr in attributeContainer.Attributes)
            {
                ...
                attribute.AttributeUpdated += (IManagedAttribute attribute) =>
                {
                    ...
                    for (int i = 0; i < AttrEventIdList.Count; i++)
                    {
                        ...
                        {
                            case 1://等于某值触发
                                if (attribute.Get<float>(AttributeValueType.Value) == paramList[1])
        ...
        /// <returns></returns>
        public FloatManagedAttribute? FindAttribute(string AttributeId)
        {
            return attributeContainer[AttributeId] as FloatManagedAttribute;
        }
        ...
        /// <param name="AttributeId"></param>
        /// <returns></returns>
        public float GetAttrNow(string AttributeId)
        {
            if (attrDict.ContainsKey(AttributeId))//有该属性
            {
                return (attributeContainer[AttributeId] as FloatManagedAttribute).Get<float>(AttributeValueType.Value);
            }
            else
            {
        ...
        /// <returns></returns>
        public float GetAttrValue(string AttributeId, AttributeValueType attributeValueType)
        {
            if (attrDict.ContainsKey(AttributeId))//有该属性
            {
                return (attributeContainer[AttributeId] as FloatManagedAttribute).Get<float>(attributeValueType);
            }
            else
            {
                Log.Error($"错误！属性id:{AttributeId}");
        ...
        /// <param name="AttributeId"></param>
        /// <returns></returns>
        public bool IsAttributeData(string AttributeId)
        {
