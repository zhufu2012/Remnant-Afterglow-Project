...
namespace Remnant_Afterglow
{
	...
	public partial class SaveLoadView : Control
	{
		...
		public TextureButton but_enter_archive;//进入创建存档的界面 按钮
		...
		public void LoadAllSaveFileUI()
		{
			...
			foreach (SaveFile file in file_list)
			{
				...
				SaveFileView saveFileUI = (SaveFileView)GD.Load<PackedScene>("res://src/core/ui/view/archive_view/SaveFile.tscn").Instantiate();
		...
		{
			but_return_pre_view = GetNode<Button>("Panel/ReturnBut");
			but_create_archive = GetNode<Button>("Panel/Panel/CreateSave");
			but_enter_archive = GetNode<TextureButton>("Panel/Button2");
		...
		{
			Log.Print("进入存档创建界面");
			SceneManager.ChangeSceneName("CreateArchiveView", this);
		}
		/// <summary>
		/// 确定进入选择的存档 按钮
		/// </summary>
		public void EnterArchive()
		{
			foreach (Panel fileView in GetNode<VBoxContainer>("Panel/Panel/ScrollContainer/VBoxContainer").GetChildren())
			{
				...
				{
					//BigTileMap bigmap = (BigTileMap)GD.Load<PackedScene>("res://src/core/map/BigMapView.tscn").Instantiate();
					SaveLoadSystem.SetNowSave(saveFileView.file);//设置当前存档
					SceneManager.PutParam("chapter_id", saveFileView.file.chapter_id);
