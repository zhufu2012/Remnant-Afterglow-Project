...
{
    public partial class BigMapCamera : Camera2D
    {
        ...
        public override void _Ready()
        {
            //初始化缩放值
            camera_zoom = Zoom;
            frame_number_label = GetNode<Label>("CanvasLayer/Control/Label");
            NoiseEdit = GetNode<Control>("CanvasLayer/Control/noise_editor");
            base._Ready();
        }
        ...
        {
            if (@event.IsActionPressed("mapview_open_noise_editor"))
            {
                if (NoiseEdit.Visible)
                {
                    NoiseEdit.Visible = false;
                }
                else
                {
                    NoiseEdit.Visible = true;
                }
            }
