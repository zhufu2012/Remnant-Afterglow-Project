	...
	public partial class ConfigLoadSystem : Node
	{
		...
		public void LoadModBaseConfigData()
		{
			foreach (var k in ModLoadSystem.loadModDict)
			{
				...
				ConfigFiles file_data = JsonConvert.DeserializeObject<ConfigFiles>(jsonText);
				List<ConfigDict> configDictList = file_data.cfg_files;
				foreach (var cfgFile in configDictList)
				{
					try
					{
						string json_str = File.ReadAllText(cfgFile.file_path);
						JArray jsonArray = JArray.Parse(json_str);
						if (jsonArray.Count > 0)
						{
							//这是一个表，这里一个字段一个字段的覆盖
		...
		/// <summary>
		/// 读取某配置表所有数据，返回Dictionary<string, Dictionary<string, object>>
		/// </summary>
		...
		/// <summary>
		/// 读取某配置 某一行的数据 返回Dictionary<string, object>
		/// </summary>
		/// <param name="fileName">配置名称</param>
		/// <param name="Index">键名称</param>
		...
		/// <summary>
		/// 读取某配置表 某一行的数据，返回Dictionary<string, object>
		/// </summary>
		/// <param name="fileName">配置名称</param>
		...
		/// </summary>
		/// <param name="fileName">配置名称</param>
		/// <param name="IndexArray">键列表</param>
		/// <returns></returns>
