...
namespace Remnant_Afterglow
{
	...
	public partial class IdGenerator : Node
	{
		...
		static IdGenerator()
		{
			...
			// 检查文件是否存在，避免读取错误
			if (FileAccess.FileExists(id_file_path))
			{
			...
			{
				// 可选：创建初始文件
				SaveData(new Dictionary<int, long>()); // 调用SaveData保存空数据
			}
		}
		...
		/// <summary>
		/// 生成对应类型的唯一id,返回的是Long类型的数据
		/// </summary>
		/// <param name="type">id类型</param>
		public static long GenerateLong(int type)
		{
			if (is_use)
			{
				long id_max = get_id_max(type, 1);
		...
		public static void SaveData(Dictionary<int, long> dict_updata)
		{
			try
			{
