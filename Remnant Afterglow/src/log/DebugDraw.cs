...
namespace JD.Utility
{
    ...
    public static partial class DebugDraw
    {
        ...
        /// </summary>
        /// <remarks>
        /// 注意：文本将在调用时立即绘制，而不是以固定间隔或帧率绘制。例如，如果从 Node._PhysicsProcess 调用此方法，
        ///由于 Node._PhysicsProcess 发生的时间步长与常规屏幕更新不同，文本会出现延迟。
        /// <see cref="Node._PhysicsProcess"/> happens at a different timestep than regular screen updates.
        /// </remarks>
        /// <param name="text">要绘制的文本</param>
        /// <param name="color">The <see cref="Color"/> 文本绘制的颜色（可选，默认为品红色）</param>
        /// <param name="size">文本的字体大小（可选，默认为16）</param>
        /// <param name="position">文本绘制在屏幕上的位置（可选，默认为左上角）</param>
        /// <returns><see langword="void"/></returns>
