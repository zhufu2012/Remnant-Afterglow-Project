...
namespace ManagedAttributes
{
    ...
    /// 一个实体拥有一个这样的属性组
    /// </summary>
    public class ManagAttrCon : IObservableList
    {
        /// <summary>
        /// 定义委托，用于处理属性更新事件
        /// </summary>
        ...
        public void ApplyDamage(float damage1, float damage2, float damage3, float damage4)
        {
            ...
            var damageHandlers = Attributes.Values
                .Where(a => a is AttrData && a.Priority > 0)
                .Cast<AttrData>()
            ...
            {
                AttrData shield = (AttrData)attribute;
        ...
        public bool IsDead()
        {
            return this[Attr.Attr_001].GetRaw<float>(AttrDataType.Value) <= 0;
        }
        ...
        {
            return (this[attrId] as AttrData)?.GetFloat(valType) ?? 0f;
        }

        public void SetFloat(int attrId, float value, AttrDataType valType = AttrDataType.Value)
        {
            (this[attrId] as AttrData)?.SetFloat(value, valType);
        }
        ...
        public void Merge(ManagAttrCon other, bool overwriteExisting = false)
        {
            if (other == null)
            {
